/*1. Staircase
- Write a function that prints a staircase according to the specs bellow
- Input: n Where 0 < n <= 100
- Output: A staircase of any special symbol (#, @, *, &, !, etc) right aligned composed of height n and width n

4 
     #
    ##
   ###
  ####
*/
function alinearEscalera(altura, ancho) {

        if (altura <= 0 || altura > 100) {
            console.log("Altura entre 1 y 100.");
            return;
    }
        if (ancho <= 0 || ancho > 100) {
            console.log("Ancho entre 1 y 100.");
            return;
    }
        for (let i = 0; i < altura; i++) {
            let escalon = "";
            for (let b = 0; b < ancho; b++) {
            if (b >= ancho - i - 1) {
            escalon += "#";
        } else {
            escalon += " ";
        }
    }
            console.log(escalon);
        }
  }
            alinearEscalera(4, 4);


/*2. Book
- You are reading a book and want to get to a certain page number inside that book since you forgot where you left it, count how many page turns you have to do untill you reach the desired page.
    - Take note that opening the book doesn't count as a page turn, and when opened, the right side of the page is always page 1.
    - When flipped, page 2 and 3 are in view, so that's pageTurns = 1, when flipped again pages 4 and 5 are in view, so that's pageTurns = 2
    - Every page, except first and last are always printed on both sides.
    2. The book is n pages long and you want to turn to page p
    3. Input: 
        - n: number of pages in book
        - p: page you want to get to
        - Constraints: 1 <= n <= 10^5, 1 <= p <= n
    4. Output: number of page turns

Sample input 
6, 2
Sample output
1
*/

function cambiarPagina(totalDePaginas, numeroDePagina) {

        if(totalDePaginas > 1 || totalDePaginas < 120){
            console.log("Ingrese el total de número de paginas");
            return;        
    }
        if (numeroDePagina >= 1 || numeroDePagina < 40){
            console.log("Ingrese el número de pagina a la que quiere llegar");
            return;
    }
        for (let t = 1; t < totalDePaginas; t++){
            let pagina = "";
        for (let n = 1; n < numeroDePagina; n++){
        if (t >= numeroDePagina) {
            pagina += "";
        }else{
            pagina+= "";
        }
    }
         console.log(t,n);
         
    }
}
         cambiarPagina(6,2);


/* Birds in migration
 diferentes especies de pájaritos, tipos de especies, cuantas veces se encuentran con esas especies  5 id diferentes (1, 2, 3, 4, 5)
que id se repitio más pero tiene menos valor. array [4,4,2,2,3] array mayor a 2 de longitud y siempre <= a 10 a la quinta potencia.
output 2, ocurre más y tiene menor valor

3. Birds in migration
- You are watching birds migrate in the country, taking notes on the different types of birds you encounter by setting their ID on an array. If more than 1 type has been spotted that maximum amount, return the smallest of their IDs.
    - Input: ```arr[4,4,2,2,3]``` 
        - Constrints: 2 < arr <= 2x10^5, where ```n``` inside the array is 0 < n <= 5
    - Output: Type: 2
        - Type 9 and 2 have two ocurrences (the maximum in the array), but 2 is the smallest of their IDs, so return 'Type: 2'

Sample input:

[1,4,4,2,1,1,1]

Sample output:

Type: 1
*/
        const id=tiposDeAves =["Dodo", "Condor", "Golondrina", "Colibrí", "Garza"];
        console.log(id);

        console.log(id.indexOf("Dodo"));
        console.log(id.indexOf("Condor"));
        console.log(id.indexOf("Golondrina"));
        console.log(id.indexOf("Colibrí"));
        console.log(id.indexOf("Garza"));
    
        const coincidencias = [1, 4, 4, 2, 1, 1, 3, 1];

function avesEnMigración (id, coincidencias) {
    if(id = 0 || id > 4 ){
        console.log("Tipos de Aves");
        return;
    }
    if(coincidencias = 0 || coincidencias < 4){
        console.log(coincidencias);
        return;
    }
        else {
        }
}
id.forEach(condor => console.log(condor));

console.log(id, coincidencias);

avesEnMigración(1.4);

